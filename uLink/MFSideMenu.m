...
@synthesize sideMenuEnabled = _sideMenuEnabled;
...
+ (MFSideMenu *) menuWithNavigationController:(UINavigationController *)controller
    ...
    ((MainNavigationViewController*)controller).sideMenu = menu;
...
- (void) navigationControllerDidAppear {
    ...
    if([self shadowEnabled]) {
        ...
        self.rootViewController.view.layer.shadowOpacity = 0.80f;
...
- (UIBarButtonItem *)backBarButtonItem {
    return [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"Icon.png"]
...
- (void) showOptionsButton {
    self.navigationController.topViewController.navigationItem.leftBarButtonItem = [self menuBarButtonItem];
}

- (void) setupSideMenuBarButtonItem {
    if (self.sideMenuEnabled) {
...
- (void) navigationControllerPanned:(id)sender {
    // only allow pans when the menu is visible
